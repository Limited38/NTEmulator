...
{
	//if not attacking
	if(!Map.MapData[all_map].Enemy[all_en].walking)
	{
		...
		if(time > Map.MapData[all_map].Enemy[all_en].random_move_next)
		{
			 ...
			 Map.MapData[all_map].Enemy[all_en].target_x = gen_x;
			 Map.MapData[all_map].Enemy[all_en].target_y = gen_y;
			 Map.MapData[all_map].Enemy[all_en].walking = true;

		}
	}else
	{
		//Check path
		Map.MapData[all_map].Enemy[all_en].x = Map.MapData[all_map].Enemy[all_en].target_x;
		Map.MapData[all_map].Enemy[all_en].y = Map.MapData[all_map].Enemy[all_en].target_y;
		Map.MapData[all_map].Enemy[all_en].walking = false;

		std::stringstream movepacket;
		movepacket << "mv " << OBJ_ENEMIES << " " << Map.MapData[all_map].Enemy[all_en].id << " " << Map.MapData[all_map].Enemy[all_en].x << " " << Map.MapData[all_map].Enemy[all_en].y << " " << 11;
		Map.SendMap(all_map, NULL, true, movepacket.str());
	}
}
...
void TEnemies::Enemy_Spawn( TSocket *Client, int vnum, int level, int count)
{
	...
	for(int counter = 0; counter < count; counter++)
	{
		...
		enemy.id = General.CreateRandomNumber(1000, 100000);
		enemy.vnum = vnum;
...
void TEnemies::Handle_Attack(int mapindex, int all_en)
{
	...
	}else
	{
		...
		if((GetTickCount() / 100) > Map.MapData[mapindex].Enemy[all_en].attack.casting_time)
		{
			...
			int resthp = (Map.MapData[mapindex].Players[Allocated_Target]->Character->Status.currenthp / Map.MapData[mapindex].Players[Allocated_Target]->Character->Status.maxhp) * 100;
			int hitmode = 0;
			int typ = 0;
